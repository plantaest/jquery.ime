( function ( $ ) {
	...
	QUnit.module( 'VIWP.IME – Phase 1 integration spike', {
		before: function ( assert ) {
			...
			$.ime.load( 'vi-vni' ).then( function () {
				done();
			}, function () {
				assert.true( false, 'Vietnamese shared source loads before spike tests run' );
				done();
			} );
		}
	} );

	QUnit.test( 'Vietnamese input methods are registered through shared source metadata', function ( assert ) {
		assert.deepEqual(
	...
	QUnit.test( 'Loading one Vietnamese method registers the shared engine and all adapters', function ( assert ) {
		assert.strictEqual(
	...
	QUnit.test( 'Vietnamese adapter maps handled engine output to jQuery.IME patterns result', function ( assert ) {
		var adapter, result;
	...
	QUnit.test( 'Vietnamese production spike adapters pass through until the engine handles commands', function ( assert ) {
		var result = $.ime.inputmethods[ 'vi-vni' ].patterns( 'a1', '' );
		...
		assert.deepEqual(
			...
			'The Phase 2 production stub does not implement Vietnamese composition behavior'
		);
		assert.strictEqual(
			$.ime.inputmethods[ 'vi-vni' ].contextLength,
			0,
			'Vietnamese spike adapters do not depend on raw input context'
		);
		assert.strictEqual(
			$.ime.inputmethods[ 'vi-vni' ].maxKeyLength,
			$.ime.vi.DEFAULT_MAX_KEY_LENGTH,
			'Vietnamese spike adapters use the shared provisional maxKeyLength'
		);
	} );
